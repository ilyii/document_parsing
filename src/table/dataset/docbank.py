...
def worker(pdf_file, data_dir, output_dir):
    try:
        ...
        pdf_images = []
        pdf = fitz.open(os.path.join(data_dir, pdf_file))
    ...
    for page_id in tqdm(range(len(pdf.pages))):
        ...
        anno_img = (
            np.ones([int(this_page.width), int(this_page.height)] + [3], dtype=np.uint8)
            * 255
        )
        ...
        for word in words:
            ...
            for obj in this_page.layout._objs:
                ...
                obj_bbox = (
                    obj.bbox[0],
                    float(this_page.height) - obj.bbox[3],
                    obj.bbox[2],
                    float(this_page.height) - obj.bbox[1],
                )
        ...
        pdf_images[page_id].save(
            os.path.join(
                output_dir,
                pdf_file.replace(".pdf", "") + "_{}_ori.jpg".format(str(page_id)),
            )
        )
        anno_img.save(
            os.path.join(
                output_dir,
                pdf_file.replace(".pdf", "") + "_{}_ann.jpg".format(str(page_id)),
            )
        )
        with open(
            os.path.join(
                output_dir,
                pdf_file.replace(".pdf", "") + "_{}.txt".format(str(page_id)),
            ),
            "w",
            encoding="utf8",
        ) as fp:
